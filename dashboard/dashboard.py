...
st.subheader("📈 Analisis Tren Penyewaan Sepeda")

fig_trend, ax_trend = plt.subplots(figsize=(12, 6))
ax_trend.plot(df["dteday"], df["cnt_hourly"], label="Total Peminjaman", color="blue", alpha=0.7)
...
df["MA_7"] = df["cnt_hourly"].rolling(window=7).mean()
df["MA_30"] = df["cnt_hourly"].rolling(window=30).mean()

fig_ma, ax_ma = plt.subplots(figsize=(12, 6))
ax_ma.plot(df["dteday"], df["cnt_hourly"], label="Total Peminjaman", color="blue", alpha=0.5)
ax_ma.plot(df["dteday"], df["MA_7"], label="Moving Average 7 Hari", color="red")
ax_ma.plot(df["dteday"], df["MA_30"], label="Moving Average 30 Hari", color="green")
...
st.subheader("🔍 Decomposition Analysis (Tren, Musiman, Residual)")

from statsmodels.tsa.seasonal import seasonal_decompose

decomposition = seasonal_decompose(df["cnt_hourly"], model='additive', period=30)

fig_decomp, axes = plt.subplots(4, 1, figsize=(12, 8))
axes[0].plot(df["dteday"], df["cnt_hourly"], label="Original", color="blue")
axes[0].legend()
axes[1].plot(df["dteday"], decomposition.trend, label="Trend", color="red")
axes[1].legend()
axes[2].plot(df["dteday"], decomposition.seasonal, label="Seasonality", color="green")
axes[2].legend()
axes[3].plot(df["dteday"], decomposition.resid, label="Residual", color="black")
axes[3].legend()
plt.tight_layout()
